...
6. **我的收藏**：記錄你已經有的卡（搜尋結果或詳情按 📦，願望清單按 📦 則是
   「買到了」直接轉進收藏）。匯入牌組後按**➖ 扣除已收藏**，會先列出每張卡
   「已有幾張／還缺幾張」讓你確認，套用後只把真正缺的留在清單去比價。
   可填購入單價，對照比價時存下的行情快照看收藏市值與損益。
...
- 收藏的市值取自比價時存下的價格快照，**沒比價過的卡沒有市值**（據實留白，不推估）
- 扣除已收藏的版本比對刻意寬鬆（一邊沒指定版本就不算衝突），故套用前一律列明細供確認
- 卡拍拍不整合：其服務條款明文禁止自動化擷取與未經授權呼叫 API（詳見 HANDOVER 第三節）
