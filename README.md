...
7. **我的牌組**：把目前的願望清單存成牌組（同名覆蓋），之後一鍵載回。
   存多套牌就能切換看各自缺什麼。
8. **一覽/搜尋顯示「$X 起」**：卡片直接標比價時記錄過的最低價（歷史快照、非即時，
   沒查過的卡不顯示）。
9. **流動性指標**：比價結果標「在售 N 張・累計成交 M」，分辨「真的便宜」與
   「根本沒人買」；貨少又無成交紀錄會標 ⚠。
10. **匯入校對**：牌組匯入不再直接塞進清單，先列出比對結果（含原始牌表行），
    模糊比對到的標 ⚠，逐項可取消勾選，確認後才加入。
11. **你最常查的卡**：依本機比價紀錄統計近 30 天查最多次的卡與期間漲跌
    （**不是市場熱門榜**，本站沒有平台級成交資料）。
12. **歷史瀏覽**：看過的卡片留一排縮圖方便回訪（存瀏覽器本機）。
13. **相對目標價**：設到價通知時可填 `-10%` 或 `-30`，以目前最低價（或 30 天均價）
    為基準換算。
